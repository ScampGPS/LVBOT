...
from automation.browser.recovery.strategies.base import (
    RecoveryContext,
    RecoveryStrategyExecutor,
)
from automation.browser.recovery.strategies.emergency_fallback import (
    EmergencyFallbackRecovery,
)
from automation.browser.recovery.strategies.full_restart import FullRestartRecovery
from automation.browser.recovery.strategies.individual_court import (
    IndividualCourtRecovery,
)
from automation.browser.recovery.strategies.partial_pool import PartialPoolRecovery
from automation.browser.recovery.types import (
    RecoveryAttempt,
    RecoveryResult,
    RecoveryStrategy,
)
...
class BrowserPoolRecoveryService:
    ...
    def __init__(self, browser_pool):
        ...
        """
        t(
            "automation.browser.browser_pool_recovery.BrowserPoolRecoveryService.__init__"
        )
    ...
    async def recover_browser_pool(
        self, failed_courts: List[int] = None, error_context: str = None
    ) -> RecoveryResult:
        """
        ...
        """
        t(
            "automation.browser.browser_pool_recovery.BrowserPoolRecoveryService.recover_browser_pool"
        )
        async with self.recovery_lock:
            ...
            if failed_courts is None:
                ...
                strategies = [
                    RecoveryStrategy.INDIVIDUAL_COURT,
                    RecoveryStrategy.PARTIAL_POOL,
                    RecoveryStrategy.FULL_RESTART,
                    RecoveryStrategy.EMERGENCY_FALLBACK,
                ]
            else:
    ...
    async def is_recovery_needed(self) -> Tuple[bool, List[int]]:
        ...
        """
        t(
            "automation.browser.browser_pool_recovery.BrowserPoolRecoveryService.is_recovery_needed"
        )
        failed_courts = []
    ...
    def get_recovery_stats(self) -> Dict[str, Any]:
        ...
        """
        t(
            "automation.browser.browser_pool_recovery.BrowserPoolRecoveryService.get_recovery_stats"
        )
        ...
        return {
            ...
            "last_recovery": (
                self.recovery_history[-1] if self.recovery_history else None
            ),
