...
class AsyncBookingExecutor:
    ...
    def __init__(self, browser_pool=None, use_natural_flow=False, experienced_mode=True):
        """
        ...
        Args:
            ...
            experienced_mode: Use fast experienced user timing (default: True)
        """
    ...
    async def _execute_booking_internal(
        ...
        """
        try:
            # Choose executor based on experienced_mode
            if self.experienced_mode:
                self.logger.info(f"Using EXPERIENCED USER mode for Court {court_number} (26.5s execution)")
                ...
                self.logger.info(f"Using STANDARD mode for Court {court_number} (40.2s execution)")
