...
def __getattr__(name: str):
    if name in {"ReservationQueue", "ReservationStatus"}:
        module = import_module("lvbot.domain.queue.reservation_queue")
    elif name in {"ReservationScheduler"}:
        module = import_module("lvbot.domain.queue.reservation_scheduler")
    elif name in {"PriorityManager", "PriorityUser"}:
        module = import_module("lvbot.domain.queue.priority_manager")
    elif name in {"ReservationTracker"}:
        module = import_module("lvbot.domain.queue.reservation_tracker")
    elif name in {"ReservationHelpers"}:
        module = import_module("lvbot.domain.queue.reservation_helpers")
    else:
